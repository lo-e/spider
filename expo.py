...
nft_main_name_re = re.compile(r'<a href="/accounts/login.*next=/catalog/(.*?)/">')

# 钉钉通知
dingtalk = DingTalkEngine()

@dataclass
class ExpoNFT(BaseSpider):
    nft_link:str = ''               # 详情链接
    image_link:str = ''             # 封面
    nft_name:str = ''               # 名称
...
def expo():
    ...
    while True:
        ...
        if html:
            ...
            # 主NFT
            main_items = soup.find_all('main', class_='main-content-wrapper')
            for item in main_items:
                item_str = str(item)
                nft_name_list = re.findall(nft_main_name_re, item_str)
                for nft_name in nft_name_list:
                    data = ExpoNFT()
                    data.nft_name = nft_name
